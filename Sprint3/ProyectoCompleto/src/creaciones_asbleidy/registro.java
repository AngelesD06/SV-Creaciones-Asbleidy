...
public class registro extends javax.swing.JFrame {
    ...
    public void Validar() {
        ...
        if (Nombre.equals("") || Email.equals("") || Contra.equals("")) {
            ...
            //Se llama al método Verificarec y se le envía el email ingresado por el usuario
            int ec = Verificarec(Email);
            //Si ya está registrado el email se le informa al usuario
            if (ec == 1) {
                JOptionPane.showMessageDialog(this, "Ya se encuentra registrado un usuario con este correo electrónico");
            } else {   
                //Si no está registrado el email se guarda la información
                Connection reg = con.getConection();
                //Se crea la sentencia para insertar la información del usuario
                String sql = "insert into usuario (email,pass,tipo,nombre) values (?, ?, ?, ?)";
                try {
                    ...
                    //Se llama al método Email_Pago y se le envía el email ingresado
                    Email_Pago(Email);
                    login l = new login();
                    l.setVisible(true);
                    dispose();
    ...
    //Método para crear un registro en los datos del pago
    public void Email_Pago(String Email) {
        Connection reg = con.getConection();
        //Método para insertar el email en los datos del pago
        String sql = "insert into datos_pago (email) values (?)";
        try {
            //Se ingresa la información en la base de datos
            PreparedStatement pst = reg.prepareStatement(sql);
            pst.setString(1, Email);
